# D:/synapse-md/synapse_md_app/llm_interface.py
import requests
import json
...
from config import (
    ACTIVE_LLM_BACKEND,
    GOOGLE_API_KEY, GOOGLE_MODELS, _google_api_key_default,
    LM_STUDIO_URL, LM_STUDIO_HEADERS, LM_STUDIO_MODELS
)
...
class LLMInterface:
    ...
    def _get_model_name(self, tier: str) -> str: raise NotImplementedError
    def generate_text(self, prompt: str, max_tokens: int = 500, temperature: float = 0.7) -> str: raise NotImplementedError
    def generate_json(self, prompt: str, json_schema: dict, max_tokens: int = 300, temperature: float = 0.5) -> dict: raise NotImplementedError

    def summarize_text(self, text: str, max_length: int = 150, temperature: float = 0.3) -> str:
        prompt = f"Summarize the following text concisely (around {max_length} words):\n\n{text}\n\nSummary:"
        estimated_tokens = int(max_length * 1.5)
        return self.generate_text(prompt, max_tokens=estimated_tokens, temperature=temperature)

    def autocomplete(self, text_before_cursor: str, max_tokens: int = 50, temperature: float = 0.5) -> str:
        prompt = f"Complete the following text:\n\n{text_before_cursor}"
        return self.generate_text(prompt, max_tokens=max_tokens, temperature=temperature).strip()
...
class GoogleAIClient(LLMInterface):
    def __init__(self, model_tier: str = "small"):
        ...
        if GOOGLE_API_KEY == _google_api_key_default:
            print("❌ GoogleAIClient: Cannot initialize, API key is missing or placeholder.")
        else:
            try:
                genai.configure(api_key=GOOGLE_API_KEY)
                self.client = genai.GenerativeModel(self.model_name)
                print(f"✅ Google AI client initialized with model: {self.model_name}")
            except Exception as e:
                st.error(f"❌ Failed to initialize Google AI client with provided key: {e}")
                print(f"❌ Failed to initialize Google AI client: {e}")

    def _get_model_name(self, tier: str) -> str: return GOOGLE_MODELS.get(tier, GOOGLE_MODELS["small"])

    def generate_text(self, prompt: str, max_tokens: int = 500, temperature: float = 0.7) -> str:
        if not self.client: return "Error: Google AI client not initialized (API key missing or invalid)."
        try:
    ...
    def generate_json(self, prompt: str, json_schema: dict, max_tokens: int = 300, temperature: float = 0.5) -> dict:
        if not self.client: return {"error": "Google AI client not initialized (API key missing or invalid)."}
        try:
            generation_config = genai.types.GenerationConfig(
                response_mime_type="application/json", max_output_tokens=max_tokens, temperature=temperature
            )
            ...
            if response.parts:
                ...
                try:
                    ...
                    return {"error": "Invalid JSON response", "raw_content": response_text, "decode_error": str(json_e)}
            if hasattr(response, 'prompt_feedback') and response.prompt_feedback.block_reason:
                return {"error": f"Content generation blocked due to {response.prompt_feedback.block_reason.name}."}
            return {"error": "Received an empty or unexpected JSON response from Google AI."}

        except Exception as e:
            st.error(f"❌ Google AI API JSON Error: {e}")
            print(f"❌ Google AI API JSON Error: {e}")
...
class LMStudioClient(LLMInterface):
    ...
    def _call_lm_studio(self, payload: dict) -> dict:
        try:
            response = requests.post(self.api_url, headers=LM_STUDIO_HEADERS, json=payload, timeout=180)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            st.error(f"❌ LM Studio connection error: {e}. Is it running at {self.api_url}?")
            print(f"❌ LM Studio connection error: {e}")
            return {"error": f"LM Studio connection failed: {e}"}
        except Exception as e:
            st.error(f"❌ Error during LM Studio call: {e}")
            print(f"❌ Error during LM Studio call: {e}")
    ...
    def generate_text(self, prompt: str, max_tokens: int = 500, temperature: float = 0.7) -> str:
        ...
        try: return result["choices"][0]["message"]["content"].strip()
        except (IndexError, KeyError, TypeError):
             print("❌ Unexpected LM Studio response format:", result)
    ...
    def generate_json(self, prompt: str, json_schema: dict, max_tokens: int = 300, temperature: float = 0.5) -> dict:
        ...
        try:
            ...
            print("❌ Unexpected LM Studio JSON response format:", result)
            return {"error": "Unexpected JSON response format from LM Studio."}
        except json.JSONDecodeError as json_e:
            return {"error": "Invalid JSON response", "raw_content": content, "decode_error": str(json_e)}
...
def get_llm_client(model_tier: str = "small") -> LLMInterface | None:
    ...
    print(f"⚙️ Getting LLM client for backend: {backend}, tier: {model_tier}")
    if backend == "google":
        client = GoogleAIClient(model_tier=model_tier)
        # Return client only if initialized successfully (API key was valid)
