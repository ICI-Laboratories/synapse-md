...
def get_relevant_context_snippets(
    ...
    for name in selected_context_names:
        ...
        if is_text_file:
            ...
            if not summary_data:
                full_text = load_extracted_text(user_identifier, name)
        source_contribution = ""
        if summary_data and "page_summaries" in summary_data:
            ...
            for page_sum in summary_data["page_summaries"]:
                ...
                if (
                    ...
                    break
            if body:
                source_contribution = header + body
        elif full_text:
            header = (
                f"\n--- Context Source: {name} "
                f"{'(Text File)' if is_text_file else '(Full Text Snippet)'} ---\n"
            )
        ...
        if source_contribution:
            ...
            approx_tokens += len(source_contribution.split())
    return context_str[: max_tokens * 7]
...
def get_inline_suggestion(current_text: str) -> str:
    ...
    context_chunk = context_chunk[-MAX_CONTEXT_TOKENS_SMALL:]
    ...
    if suggestion.startswith(context_chunk):
        suggestion = suggestion[len(context_chunk) :].strip()
    return suggestion.split("\n")[0]


def get_sidebar_suggestions(
    user_identifier: str, current_draft: str, selected_context_names: list[str]
) -> list[dict]:
    suggestions = []
    ...
    response_data = llm_medium.generate_json(
        prompt, json_schema, max_tokens=1000, temperature=0.7
    )
    if isinstance(response_data, list):
        ...
        if "raw_content" in response_data:
            st.code(response_data["raw_content"], language="json")
    return suggestions


def generate_initial_draft(
    user_identifier: str, draft_prompt: str, selected_context_names: list[str]
) -> str:
    ...
    if not llm_large:
        return "Error: Large LLM client unavailable."
    context_limit = MAX_CONTEXT_TOKENS_LARGE - len(draft_prompt.split()) - 500
    context_material = get_relevant_context_snippets(
        user_identifier,
        selected_context_names,
        draft_prompt,
        context_limit if context_limit > 0 else 500,
    )
    full_prompt = (
    ...
    draft_content = llm_large.generate_text(
        full_prompt, max_tokens=3000, temperature=0.7
    )
